...
\section{Remote Teams and Scrum@Scale}
Scrum@Scale works without modification for remote teams.  Scrum@Scale can run, lead, and unify remote teams and remote team members.  Having remote teams introduces known risks.  These risks comes from the challenges:
\begin{itemize}
...
digitization of physical artifacts
\end{itemize}
These risks, and others, are manageable.  What follows are best practices to bring extra discipline and thoughtfulness.  Without extra thoughtfulness and special considerations, remote teams will under-perform.  

The future or work is software enabled, high bandwidth, digitized, and distributed.  Let's navigate the future of distributed work as tools, technology, and connectivity improve. 

Scrum@Scale endorses no software, platform, or service.  Your guiding principle is simple.  Tools and techniques serve the teams.  Where tools do not meet team needs, switch tools.  No tool is more important than the team that wields it.
...
Meeting times
Meeting procedures
Limits of individual decision authority
Domain specific requirements
Cameras always on/sometimes one/never on
...
Swarming is a key pattern for Scrum teams.  Swarming is challenging with distributed teams.  Teams should consider a time-box immediately following the Daily Scrum for swarming.  This one hour time-box should be available for issues that emerge during the Daily Scrum.  If more than one issue emerges as a swarming candidate, the Scrum Master should guide the team.  Teams may opt to swarm both issues in series (one after the other) or in parallel (at the same time).  If teams swarm in parallel, breakout rooms are a best practice.
Enterprise tools are important to distributed teams.  Remote teams should have equal access to enterprise tools at all locations.  Remote teammates should have equal access to all digital tools.  Digital tools unite a team and define workflows.  Having a common tool-set will ensure a consistent look and feel to work.  It will also ensure that teams are able to transfer tool learning.  
Scrum@Scale works in part because it results in increased safety and team trust.  Remote working requires extra effort to build and maintain this trust.  Teams should seek ways to build trust and look for signs of trust erosion.  Trust erosion and low trust teams 
 \begin{itemize} 
have low happiness
under perform
...
Leading remote Scrum@Scale requires empirical process control.  To divide resources leaders must be able to compare teams.  Leaders must maximize value by investing in the greatest return on investment.  Talent is global and distributed teams can deliver attractive value.  Remote teams are also need informed and active leadership.  Scrum@Scale provides the framework to lead teams without modification.  Remote teams are an emergent phenomenon that integrate well with Scrum@Scale.  Leaders must understand the current state of trust and increase trust. 
Leaders must also use the EMS and EAT to communicate with all teams.  Live, video communication with chat is richest for remote teams.  Emerging technology may offer richer communication in the future.
Scrum teams are autonomous because high performance is possible on the edge of chaos.  Enterprise leaders must resolve the tension between digital tools and team autonomy.  Autonomous teams should be able to select tools that serve them best.  Enterprise software licenses are expensive, and offering many tools is costly.  Leadership must balance the needs of teams with enterprise resources through the EAT.  No tool is worth undue financial strain.  In instances where tools do not meet team needs, teams should adjust to commercial tools.  Tailoring tools is not cost effective and creates high cyber security risks.  The tool that is most secure is the tool that has the latest software update.  AI exploits tools in near real time.  Few enterprises have the resources to conduct DevSecOps on team collaboration tools.  Team collaboration tools safeguard intellectual property and key trade advantages.  The EAT must manage the autonomy of teams, resources, and cyber security risks. 
\subsection{Connecting remote teams with technology}
\section{End Note}
