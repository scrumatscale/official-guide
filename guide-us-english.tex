...
Product Owner Teams hold a stakeholder alignment meeting, the \textbf{MetaScrum Event}, on a regular cadence, at least once a sprint. 
\begin{itemize}
\item Each team PO (or proxy) attends the MetaScrum Event where the Chief Product Owner presents the product backlog to business owners who control funding, personnel, and customer committments. They address any changes needed to strategy, funding, resource allocation, and deplyments, and collaborate with the Chief Product Owner's to build an agreement on a Product Backlog that they will support until the next MetaScrum.
